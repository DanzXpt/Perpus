...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6 p-4">
        <div class="flex items-center justify-between">
            ...
            <div
                class="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-2xl text-[10px] font-bold uppercase border border-indigo-100 tracking-widest">
                ID: #{{ $user->id }}
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    <h3 class="text-xs font-bold text-indigo-500 uppercase tracking-widest border-b border-slate-50 pb-2">
                        Informasi Akun</h3>
                    ...
                    <div class="flex flex-col space-y-1">
                        <span class="text-xs text-slate-400 font-medium italic">role Akses</span>
                        <div>
                            <span
                                class="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-[10px] font-bold uppercase tracking-tighter">
                                {{ ucfirst($user->role) }}
                            </span>
                        </div>
                    </div>
                ...
                <div class="space-y-6 bg-slate-50/50 p-6 rounded-3xl border border-slate-100">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-2">
                        Data Spesifik</h3>
                    ...
                    <div class="pt-4 border-t border-slate-200 flex justify-between">
                        <div class="text-[10px] text-slate-400">
                            Dibuat: <span
                                class="text-slate-600 font-medium">{{ optional($user->created_at)->format('d M Y') }}</span>
                        </div>
                        <div class="text-[10px] text-slate-400 text-right">
                            Update: <span
                                class="text-slate-600 font-medium">{{ optional($user->updated_at)->format('d M Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-10 pt-6 border-t border-slate-100 flex flex-wrap gap-3">
                ...
                <form action="{{ route('kepala.akun.destroy', $user->id) }}" method="POST" class="inline">
                    @csrf @method('DELETE')
                    <button type="submit"
                        onclick="return confirm('Hapus akun ini? Data terkait mungkin akan ikut terhapus.')"

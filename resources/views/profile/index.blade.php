        ...
        <div class="bg-white rounded-[3rem] shadow-2xl shadow-indigo-100 overflow-hidden border border-slate-100">
            ...
            <div class="bg-slate-900 p-12 text-white relative overflow-hidden">
                ...
                <div class="relative flex items-center gap-8">
                    <div
                        class="w-24 h-24 bg-indigo-500 rounded-[2rem] flex items-center justify-center text-4xl font-black shadow-xl border-4 border-slate-800">
                        {{ substr(Auth::user()->name, 0, 1) }}
                    </div>
                    <div>
                        <h1 class="text-3xl font-black uppercase tracking-tighter italic italic">{{ Auth::user()->name }}
                        </h1>
                        <span
                            class="bg-indigo-500/20 text-indigo-400 px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest border border-indigo-500/30">
                            role: {{ Auth::user()->role }}
                        </span>
                    </div>
                </div>
            ...
            <div class="p-12 space-y-8">
                ...
                <div class="pt-8 border-t border-slate-100 flex gap-4">
                    <button
                        class="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200">
                        Edit Profil
                    </button>
                    <button
                        class="bg-slate-100 text-slate-600 px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-200 transition-all">
                        Ganti Password
                    </button>
                </div>
            </div>
